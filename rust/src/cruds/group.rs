...
pub async fn make_invitation(user_id:String,group_id:Uuid)->Result<HttpResponse,Error>{
    let db =  get_db().await;
    let invitation_exist = match invite_groups::Entity::find_by_id((user_id.clone(),group_id)).one(&db).await{
        Ok(value) => value,
